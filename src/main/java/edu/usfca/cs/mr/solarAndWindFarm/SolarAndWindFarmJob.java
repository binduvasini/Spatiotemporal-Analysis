 ...
 */
public class SolarAndWindFarmJob {
    public static void main(String[] args) {
        try {
            Configuration conf = new Configuration();
            // Give the MapRed job a name. You'll see this name in the Yarn
            // webapp.
            Job job = Job.getInstance(conf, "solar + wind farm job");
            // Current class.
            job.setJarByClass(SolarAndWindFarmJob.class);
            // Mapper
            job.setMapperClass(SolarAndWindFarmMapper.class);
            // Combiner. We use the reducer as the combiner in this case.
            //job.setCombinerClass(SolarAndWindFarmReducer.class);
            // Reducer
